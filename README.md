...
Static mode can be enabled in settings. In static mode P is set to 1 second and the game starts paused with one coin in the purse. Dragging a coin into a different box advances the game by one P, except feeding the robot costs no time. Shrink and split progress is counted in moves instead of real seconds. Particles keep floating while game time is paused.
